...
int* mas;
int* mas_seq_bubble; // –-т последовательной версии
int* mas_seq_qsort;
int* mas_pp_bubble; // –-т параллельной версии
int* mas_seq_merge;
int* mas_pp_merge;

int** matr_merge_pp; // «аранее выдел€ем пам€ть дл€ потоков, которые будут сортировать локальные буферы (чтобы не тратить врем€ на выделение и освобождение пам€ти)
int* mas_merge_seq;
...
/* ћеханизм синхронизации потоков */
volatile bool* ThreadIsFinish;

// —оздать и проинициализировать массив
int* Create_and_init_arr(int size_arr)
{
    ...
    if (size_arr < 5)// ≈сли выполн€етс€, то вводим вручную
...
// ѕузырькова€ сортировка методом чет-нечетной перестановки (последовательна€ верси€)
void BubbleSort_seq(int* arr)
{
    for (int i = 0; i < Size; i++)
        if (i % 2 == 1) // нечетна€ итераци€
        {
...
// Ѕыстра€ сортировка (последовательна€ верси€)
void QSort_seq()
{
...
void MergeSplit_right(int index_thread, int start_pos, int& end_pos, int work, int i)
{
    ...
    /* —инхронизаци€ */
    while (!ThreadIsFinish[index_thread + (int)pow(2, i)]); // ќжидаем поток, с которым будем сливать массивы
    ...
    end_pos = end_pos_right + 1; // ” этого потока массив увеличилс€ до размера size_merge_mas => необходимо изменить end_pos

    if (index_thread % (int)pow(2, i + 1) == 0 && index_thread % (int)pow(2, i + 2) != 0)
        ThreadIsFinish[index_thread] = true; // “о этот поток больше работать не будет, он выполнил свою задачу
...
// ѕузырькова€ сортировка методом чет-нечетной перестановки (параллельна€ верси€)
...
void main(int argc, char* argv[])
{
    ...
    int Show = 0;
    bool IsEnd = false; // ≈сли true, то выход из меню
    ...
    while (!IsEnd)
    {
        ...
        case 3:
            if (PowerOfTwo(num_of_threads))
            {
                ...
                for (int i = 0; i < num_of_threads; i++)
                    mas_of_threads[i] = thread::thread(BubbleSort_pp, &index_thread[i]);
        ...
        case 5:
            ...
            if (PowerOfTwo(num_of_threads))
            {
                ...
                for (int i = 0; i < num_of_threads; i++)
                    mas_of_threads[i] = thread::thread(MergeSort_pp, &index_thread[i]);
